...
cat /tmp/temp_gfwlist | sed -e '/^$/d' > conf/domain-set/proxy-domain-list.conf
...
echo -e "${domain_list}" | sort | uniq |sed -e 's/#.*//g' -e '/^$/d' -e 's/server=\///g' -e 's/\/114.114.114.114//g' | sort -u > conf/domain-set/direct-domain-list.conf
